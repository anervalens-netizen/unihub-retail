...
class _GenerationFiles:
    ...
    rule_sources_manifest: list[dict[str, str]]
    rule_sources_sha256: str
...
def _read_rule_source(raw_path: str, expected_sha256: object) -> tuple[bytes, str, str]:
    unresolved = Path(raw_path)
    source = resolve_path(raw_path, get_repo_root())
    if unresolved.is_symlink() or source.is_symlink() or not source.is_file():
        raise ValueError("Masterul regulii promo lipsește")
    content = source.read_bytes()
    digest = hashlib.sha256(content).hexdigest()
    if expected_sha256 not in {None, "", digest}:
        raise ValueError("Masterul regulii promo nu corespunde hashului aprobat")
    return content, digest, source.suffix.casefold()


def _collect_rule_sources(config: dict) -> tuple[_RuleSource, ...]:
    grouped: dict[str, tuple[bytes, str, set[str]]] = {}
    for promotion in config.get("promotions", []):
        raw_path = promotion.get("source_file") if isinstance(promotion, dict) else None
        if not raw_path:
            continue
        original = str(raw_path)
        content, digest, suffix = _read_rule_source(
            original,
            promotion.get("source_sha256"),
        )
        previous = grouped.setdefault(digest, (content, suffix, set()))
        if previous[0] != content:
            raise ValueError("Coliziune de hash pentru masterul regulii promo")
        previous[2].add(original)
    return tuple(
        _RuleSource(
            original_paths=tuple(sorted(originals)),
            name=f"promo_rule-{digest}{suffix}",
            content=content,
            sha256=digest,
        )
        for digest, (content, suffix, originals) in sorted(grouped.items())
    )


def _materialize_rule_config(
    config: dict,
    generation_dir: Path,
    sources: tuple[_RuleSource, ...],
) -> dict:
    result = json.loads(json.dumps(config))
    source_by_original = {
        original: source
        for source in sources
        for original in source.original_paths
    }
    for promotion in result.get("promotions", []):
        raw_path = promotion.get("source_file") if isinstance(promotion, dict) else None
        if not raw_path:
            continue
        source = source_by_original[str(raw_path)]
        promotion["source_file"] = str(generation_dir / source.name)
        promotion["source_sha256"] = source.sha256
    return result
...
def _rule_manifest(
    ...
    return [
        {"file": str(generation_dir / source.name), "sha256": source.sha256}
        for source in sources
    ]
...
def _write_durable_private_file(path: Path, content: bytes) -> None:
    ...
    try:
        ...
        while view:
            ...
            view = view[written:]
        os.fchmod(descriptor, 0o660)  # nosec: B103 - shared-group promo artifact boundary
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
...
def _prepare_generation_files(
    *,
    ...
    rule_identity = _canonical_json_bytes(
        {
            "rule_sources": [
    ...
    owned_config = _materialize_rule_config(config, generation_dir, rule_sources)
    for promotion in owned_config["promotions"]:
    ...
    config_bytes = _canonical_json_bytes(owned_config)
    config_sha256 = hashlib.sha256(config_bytes).hexdigest()
    rule_sources_manifest = _rule_manifest(generation_dir, rule_sources)
    return _GenerationFiles(
        ...
        rule_sources_manifest=rule_sources_manifest,
        rule_sources_sha256=hashlib.sha256(
            _canonical_json_bytes({"rule_sources": rule_sources_manifest})
        ).hexdigest(),
    )
...
def _verify_existing_generation(files: _GenerationFiles) -> None:
    ...
    expected = (
        ...
        *tuple(
            (files.generation_dir / source.name, source.sha256)
            for source in files.rule_sources
        ),
    )
    if any(
        not path.is_file()
    ...
    for path, _ in expected:
        path.chmod(0o660)  # nosec: B103 - shared-group promo artifact boundary
...
def _write_generation(
    ...
    *,
    ...
    for source in files.rule_sources:
        _write_durable_private_file(staging / source.name, source.content)
...
def _pointer_hashes(files: _GenerationFiles) -> dict:
    return {
        ...
        "rule_sources": files.rule_sources_manifest,
        "rule_sources_sha256": files.rule_sources_sha256,
    }
...
def _publish_locked(
    ...
    *,
    ...
    if pointer_bytes is not None:
        pointer_path.chmod(0o660)  # nosec: B103 - shared-group promo artifact boundary
    current_hash = (
    ...
    *,
    ...
    try:
        with lock_path.open("a+b") as lock_file:
            os.fchmod(lock_file.fileno(), 0o660)  # nosec: B103 - 0660 is intentional shared-group mode
