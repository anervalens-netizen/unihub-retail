...
_BUCHAREST_TIMEZONE = ZoneInfo("Europe/Bucharest")
...
_PROMOTION_SQL_1 = """
            SELECT id, import_month, cutoff_date, manifest, manifest_sha256, source_sha256,
...
def _sales_event_cutoff(value: object) -> datetime:
    if isinstance(value, datetime):
        if value.tzinfo is None:
            raise SalesGenerationValidationError("Sales generation cutoff must be timezone-aware")
        return value.astimezone(timezone.utc)
    if isinstance(value, date):
        local_midnight = datetime.combine(
            value,
            time.min,
            tzinfo=_BUCHAREST_TIMEZONE,
        )
...
async def promote_sales_generation(
    ...
    *,
    ...
    async with conn.transaction():
        ...
        from repositories.transactional_outbox import emit_sales_generation_promoted

        occurred_at = await conn.fetchval("SELECT clock_timestamp()")
        await emit_sales_generation_promoted(
            conn,
            aggregate_id=f"sales-{import_month}",
            generation_hash=expected_manifest_sha256,
            source_hash=str(row["source_sha256"]),
            cutoff=_sales_event_cutoff(row["cutoff_date"]),
            month=import_month,
            revision=revision,
