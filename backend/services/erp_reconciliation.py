    ...
    *,
    ...
    for store_key in sorted(db_store_keys - report_store_keys):
        add_issue(
            ...
            scope="store",
            site_code=store_key[0],
            entity=db_stores[store_key]["locatie"],
            metric="coverage",
    ...
    for agent_key in sorted(report_agent_keys & db_agent_keys):
        ...
        raw_receipts = Decimal(
            str(receipt_rows.get(agent_key, {}).get("all_receipts", 0))
        )
    ...
    receipt_status = (
        "ok"
        if _sum(report_agents, "NrBonuri") == positive_receipt_total
        else "explained"
        if _sum(report_agents, "NrBonuri") == raw_receipt_total
    ...
    metrics = [
        _metric(
            "target",
            "Target",
            _sum(report_stores, "AccValTarget"),
            _sum(retail_stores, "target_value"),
            "RON",
        ),
        _metric(
            "sales",
        ...
        _metric(
            "quantity",
            "Cantitate accesorii",
            _sum(report_agents, "AccQttyRealizat"),
            _sum(retail_agents, "total_quantity"),
            "buc",
        ),
        _metric(
            "receipts",
            "Bonuri",
            _sum(report_agents, "NrBonuri"),
            positive_receipt_total,
            "bonuri",
            status_value=receipt_status,
            note=(
                f"ERP include {int(return_only_total)} bonuri exclusiv de retur; "
                "cardul Retail le exclude intentionat."
                if receipt_status == "explained"
                else None
            ),
        ),
        _metric(
            "receipt_2plus",
            "Bonuri cu minimum 2 accesorii",
            _sum(report_agents, "NrBon2Acc"),
            _sum(retail_agents, "receipt_2plus_count"),
            "bonuri",
        ),
        _metric(
            "focus",
            "Produse Focus",
            _sum(report_agents, "AccFocusQtty"),
            _sum(retail_agents, "focus_quantity"),
            "buc",
        ),
        _metric(
            "focus_audio",
            "Focus audio",
            _sum(report_agents, "Audio"),
            _focus_quantity(focus_rows, "Casti intraauriculare"),
            "buc",
        ),
        _metric(
            "focus_battery",
            "Focus baterii externe",
            _sum(report_agents, "Battery"),
            _focus_quantity(focus_rows, "Baterie Externa"),
            "buc",
        ),
        _metric(
            "focus_supports",
            "Focus suporturi",
            _sum(report_agents, "Suporti"),
            _focus_quantity(focus_rows, "Suport auto", "Suport telescopic"),
            "buc",
        ),
        _metric(
            "films",
            "Folii total",
            _sum(report_agents, "FoliiQtty"),
            _category_quantity(category_rows, category="Folii Sticla")
            + _category_quantity(category_rows, category="Folii TPU"),
            "buc",
        ),
        _metric(
            "glass_films",
            "Folii sticla",
            _sum(report_agents, "Folii Sticla"),
            _category_quantity(category_rows, category="Folii Sticla"),
            "buc",
        ),
        _metric(
        ...
        _metric(
            "style_protection",
            "Stil si protectie",
            _sum(report_agents, "Still&Protectie"),
            _category_quantity(
                category_rows,
                category="Stil si Protectie",
                subcategories={"Capac protectie", "Husa protectie"},
            ),
            "buc",
        ),
        _metric(
            "charging_transfer",
            "Incarcare si transfer",
            _sum(report_agents, "Incarcare&Transfer"),
            _category_quantity(
                category_rows,
                category="Incarcare si Transfer",
                exclude_subcategories={"Baterie Externa"},
            ),
            "buc",
        ),
    ]
    ...
    app_only_metrics = [
        ErpReconciliationAppMetric(
            key="promo_units",
            label="Unitati promo",
            value=campaign_value("promo_qty"),
            unit="buc",
            note=unavailable_note,
        ),
        ErpReconciliationAppMetric(
            key="incentive_sold_units",
            label="Unitati vandute in mecanismele Incentive",
            value=campaign_value("incentive_sold_qty"),
            unit="buc",
            note=unavailable_note,
        ),
        ErpReconciliationAppMetric(
            key="incentive_eligible_units",
            label="Unitati eligibile dupa promo",
            value=campaign_value("incentive_qty"),
            unit="buc",
            note=unavailable_note,
        ),
        ErpReconciliationAppMetric(
            key="incentive_qualified_units",
            label="Unitati in magazine calificate",
            value=campaign_value("incentive_qualified_qty"),
            unit="buc",
            note=unavailable_note,
        ),
        ErpReconciliationAppMetric(
            ...
            value=campaign_value("incentive_value"),
            unit="RON",
            note=unavailable_note,
        ),
    ]
