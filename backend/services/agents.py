...
from services import agent_legacy_evaluation
...
class AgentsService:
    ...
    async def get_agent_evaluation(
        ...
        return await agent_legacy_evaluation.get_agent_evaluation(
            self.repo, month, months, firma, asm, site_code
        )
    ...
    async def get_agent_evaluation_v2(
        ...
        items = [build_agent_evaluation_v2_row(row) for row in rows]
    ...
    async def get_agents_list(
        ...
        query = f"""
            ...
            SELECT
                ...
                CASE WHEN roster.agent_code IS NOT NULL THEN COALESCE(roster.home_site_code, 'TL') END AS home_site_code,
            ...
            LEFT JOIN stores home ON home.site_code=roster.home_site_code
            LEFT JOIN LATERAL (
