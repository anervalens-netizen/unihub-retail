...
def render_requests(snapshot, template, current, synced_at):
    ...
    for source in template['sheets']:
        ...
        for outrow, inrow in enumerate(rowmap):
            ...
            # Multi-line supplements and leave entries must remain completely visible.
            lines = max((str(cells.get(title, {}).get((outrow,c), '')).count('\n')+1 for c in range(cols)), default=1)
            if lines > 1:
                height = max(height, min(1000, lines * 19 + 10))
            requests.append({'updateDimensionProperties': {'range': {'sheetId':sid,'dimension':'ROWS','startIndex':outrow,'endIndex':outrow+1},'properties':{'pixelSize':height},'fields':'pixelSize'}})
        ...
        # Preserve native merges, including the owner's vertically merged KPI values.
        merges = []
        for old in source.get('merges', []):
            ...
            for outrow, inrow in enumerate(rowmap):
                if inrow == start and rowmap[outrow:outrow+end-start] == list(range(start,end)):
                    new = {**old,'sheetId':sid,'startRowIndex':outrow,'endRowIndex':outrow+end-start}
                    merges.append(new)
        if title == 'Calendar':
        ...
        # Keep unchanged native merges: imported headers may span a frozen column.
        # Recreating those merges is rejected by the API even though the copy is valid.
