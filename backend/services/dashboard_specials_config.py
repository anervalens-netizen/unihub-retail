...
def _read_generation_pointer(
    pointer_path: Path,
) -> tuple[str, str, list[Any], list[Any], list[Any], str]:
    try:
        ...
        rule_sources_manifest = pointer["rule_sources"]
        rule_sources_sha256 = str(pointer["rule_sources_sha256"])
    ...
    return (
        ...
        actuals_manifest,
        actuals_material_manifest,
        rule_sources_manifest,
        rule_sources_sha256,
    )
...
def _verify_rule_files(files: dict[str, str], generation_dir: Path) -> None:
    for filename, expected_sha256 in files.items():
        unresolved = Path(filename)
        path = resolve_path(filename, get_repo_root())
        valid = (
            not unresolved.is_symlink()
            and not path.is_symlink()
            and path.is_file()
            and path.parent.resolve() == generation_dir.resolve()
            and hashlib.sha256(path.read_bytes()).hexdigest() == expected_sha256
        )
        if not valid:
            raise ValueError(
                "Masterul regulii promo nu corespunde hashului aprobat."
            )


def _manifest_sha256(key: str, manifest: list[Any]) -> str:
    content = (
        json.dumps(
            {key: manifest},
            ensure_ascii=False,
...
def _generated_config_path(data_dir: Path) -> Path | None:
    ...
    ) = _read_generation_pointer(pointer_path)
    unresolved_candidate = generation_root / relative
    candidate = unresolved_candidate.resolve()
    root = generation_root.resolve()
    if unresolved_candidate.is_symlink() or candidate.parent.parent != root or not candidate.is_file():
    ...
    if (
        len(expected_rules) != len(rule_manifest)
        or set(expected_rules) != declared_rules
        or _manifest_sha256("rule_sources", rule_manifest)
        != expected_rule_manifest_sha256
    ):
        raise ValueError("Manifestul masterelor promo nu corespunde configului aprobat.")
    _verify_source_files(expected_sources, material=False)
    _verify_source_files(expected_materials, material=True)
    _verify_rule_files(expected_rules, candidate.parent)
    return candidate
