...
from agents.sandbox import EnvEntry, EnvValue, Environment, LocalSnapshot, Manifest, SandboxAgent, SandboxRunConfig
...
from ai_assistant.settings import AI_READONLY_DSN_ENV, AiAssistantSettings, load_ai_assistant_settings, resolve_storage_key
...
class ActiveRun:
    sandbox: Any
    result: Any
    pending_steers: deque[RuntimeSteerRequest] = field(default_factory=deque)
...
class AiSandboxRuntime:
    ...
    async def _collect_artifacts(
        ...
        for relative, digest in sorted(after.items()):
            ...
            artifact_id = uuid4()
            filename = Path(relative).name or f"artifact-{artifact_id}.bin"
            storage_key = (
    ...
    async def steer(self, conversation_id: UUID, request: RuntimeSteerRequest) -> None:
        async with self._lock:
            ...
            active.pending_steers.append(request)
    ...
    async def stream_turn(self, request: RuntimeTurnRequest) -> AsyncIterator[bytes]:
        ...
        try:
            async with self._lock:
                already_active = conversation_id in self._active
            if already_active:
            ...
            result = Runner.run_streamed(
                agent,
                _owner_input(request.text, request.current_view, upload_names),
                run_config=run_config,
                previous_response_id=request.previous_response_id,
                max_turns=None,
            )
            active = ActiveRun(sandbox=sandbox, result=result)
            async with self._lock:
            ...
            while True:
                ...
                if pending:
                    ...
                    for steer in pending:
                        names = await self._stage_request_files(
                            sandbox,
                            current_view=steer.current_view,
                            uploads=steer.uploads,
                        )
                        steer_input = _owner_input(steer.text, steer.current_view, names)
                ...
                final_text = result.final_output if isinstance(result.final_output, str) else "".join(accumulated)
                artifacts = await self._collect_artifacts(conversation_id, sandbox, before_outputs)
                yield _ndjson(
                    {
                        "type": "complete",
                        "text": final_text,
            ...
            if sandbox is not None:
                try:
                    ...
                    logger.exception("AI sandbox cleanup failed conversation=%s", conversation_id)
