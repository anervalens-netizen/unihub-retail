...
CNP = "1960523420018"
BEARER = "Bearer telemetry-secret-token"
...
FORBIDDEN = (
    ...
    f"https://retail.invalid/api/jobs/{EXPORT_ID}?secret={QUERY_SECRET}",
)
...
def _private_message() -> str:
    return (
        ...
        f"url=https://retail.invalid/api/jobs/{EXPORT_ID}?secret={QUERY_SECRET}"
    )
    ...
    try:
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://contract.invalid") as client:
            response = await client.get(
                f"/api/items/{OPERATION_ID}?secret={QUERY_SECRET}",
...
async def test_slow_and_global_exception_logs_use_route_template_not_raw_path(
    ...
    monkeypatch.setattr(main.logger, "warning", lambda *_args, **kwargs: slow_calls.append(kwargs))
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://contract.invalid") as client:
        response = await client.get(f"/api/jobs/{EXPORT_ID}?secret={QUERY_SECRET}")
    ...
    monkeypatch.setattr(main.logger, "error", lambda *_args, **kwargs: error_calls.append(kwargs))
...
def test_structured_formatter_and_postgres_sink_remove_identifier_corpus() -> None:
    record = _record(
        ...
        duration_ms=12.5,
        service_role="web",
        referer=f"https://retail.invalid/visits/{VISIT_ID}?secret={QUERY_SECRET}",
    ...
    assert db_payload["extra"]["route_template"] == "/api/visits-report/visit/{visit_id}"
...
def test_sentry_event_transaction_url_spans_and_breadcrumbs_are_canonicalized() -> None:
    event = {
        "request": {
            "url": f"https://retail.invalid/api/jobs/{EXPORT_ID}?secret={QUERY_SECRET}",
