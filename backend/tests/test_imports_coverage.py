...
async def test_campaign_reporting_publication_hook_logs_queued_job(
    ...
    pool = SimpleNamespace(
        fetchrow=AsyncMock(
            return_value={"generation_hash": generation_hash, "revision": 7}
        )
    )
    monkeypatch.setattr(
        imports_module,
...
@pytest.mark.asyncio
async def test_campaign_reporting_publication_skips_missing_sales_head(
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
) -> None:
    enqueue = AsyncMock()
    ...
    with caplog.at_level("WARNING", logger="services.imports"):
        await imports_module.trigger_campaign_reporting_publication(
            "2026-06",
            pool=cast(asyncpg.Pool, pool),
            requested_by_sub="system:promo-actuals",
            reason="promo_actuals_generation:test",
        )

    enqueue.assert_not_awaited()
    assert "sales head missing month=2026-06" in caplog.text


@pytest.mark.asyncio
async def test_campaign_reporting_publication_swallows_invalid_lineage(
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
) -> None:
    enqueue = AsyncMock()
    pool = SimpleNamespace(
        fetchrow=AsyncMock(
            return_value={"generation_hash": "not-a-sha256", "revision": 7}
        )
    )
    monkeypatch.setattr(
        imports_module,
    ...
    with caplog.at_level("ERROR", logger="services.imports"):
        await imports_module.trigger_campaign_reporting_publication(
            "2026-06",
            pool=cast(asyncpg.Pool, pool),
            requested_by_sub="system:sales-import",
    ...
    enqueue.assert_not_awaited()
    assert "enqueue campaign reporting publication esuat pentru 2026-06" in caplog.text
...
async def test_sales_import_requeues_exact_validated_generation_to_worker(
    ...
    repo.get_validated_sales_generation.assert_awaited_once()
...
@pytest.mark.asyncio
async def test_sales_import_recovers_validated_generation_in_place(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    recovered = {
        "source_spool_path": "/tmp/spool/recovered.source",
        "source_artifact_required": False,
        "source_artifact_state": "artifact_missing",
    }
    repo = MagicMock()
    repo.get_validated_sales_generation = AsyncMock(return_value=recovered)
    content = b"recovered sales bytes"
    recovered_status = SimpleNamespace(job_id="sales-import:recovered", status="completed")
    recover = AsyncMock(return_value=recovered_status)
    monkeypatch.setattr(imports_module, "recover_validated_sales_import", recover)
    enqueue = AsyncMock()
    monkeypatch.setattr(imports_module, "enqueue_sales_import", enqueue)
    pool = MagicMock()
    ...
    assert result is recovered_status
    enqueue.assert_not_awaited()
    ...
    await_args = recover.await_args
    assert await_args is not None
    assert await_args.args == (recovered,)
    assert await_args.kwargs["content"] == content
    assert await_args.kwargs["source_sha256"] == hashlib.sha256(content).hexdigest()
    assert await_args.kwargs["pool"] is pool
