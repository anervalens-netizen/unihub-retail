...
LOCKED_MIGRATION_SHA256 = "e2f3f1ff09c14cbba577ac40ba476250a7b746ebbe0bdf373c6ebf4e552fc8a0"  # noqa: synthetic test fixture SHA, not a credential  # pragma: allowlist secret
...
class MemoryOutboxRepository:
    ...
    async def record_receipt(
        ...
        *,
        ...
        del received_at
        self._claimed(event_id, owner, epoch)
    ...
    async def renew_lease(
        ...
        *,
        ...
        if event.lease_until is None or event.lease_until <= now:
            raise RuntimeError("stale outbox claim")
        event.lease_until = now + timedelta(seconds=lease_seconds)
        return True
...
def test_release_b_outbox_public_surface_and_locked_schema() -> None:
    ...
    manifest = json.loads((ROOT / "backend/db/migrations/manifest.json").read_text())
...
def test_claim_implementation_uses_skip_locked_and_finite_lease() -> None:
    ...
    repository_module = inspect.getmodule(repository_type)
    assert repository_module is not None
    ...
    assert normalized.count("LEASE_UNTIL >") >= 4


@pytest.mark.asyncio
async def test_lease_renewal_and_terminal_transitions_are_fenced() -> None:
    repository = MemoryOutboxRepository()
    now = datetime.fromisoformat("2026-08-13T08:00:00+00:00")
    event = repository.seed(now=now, name="lease-fence")
    assert await repository.claim_batch(
        owner="operations-lease",
        now=now,
        limit=1,
        lease_seconds=60,
    ) == [event]
    ...
    assert event.lease_until == now + timedelta(seconds=90)
...
def test_sales_promotion_emits_inside_its_existing_transaction() -> None:
    source = (ROOT / "backend/services/sales_generation_flow.py").read_text(
        encoding="utf-8"
    )
...
async def _exercise_emit_dedup(
    connection: Any,
    emit: Any,
    cutoff: datetime,
    occurred_at: datetime,
    ...
    await connection.execute("RESET ROLE")
    return first_id, second_id, other_id


async def _exercise_key_order_and_privacy(
    connection: Any,
    generation_hash: str,
) -> None:
    ...
    payload = _decode_json(rows[0]["payload"])
...
async def _exercise_web_read_role_denial(
    connection: Any,
    repository: Any,
    emit: Any,
    cutoff: datetime,
...
async def _exercise_head_of_line_claim(
    connection: Any,
    repository: Any,
    first_id: UUID,
    second_id: UUID,
...
async def test_emit_rollback_key_order_privacy_roles_and_head_of_line_claim() -> None:
    ...
    try:
        ...
        first_id, second_id, other_id = await _exercise_emit_dedup(
            connection,
            emit,
            cutoff,
            occurred_at,
        ...
        await _exercise_key_order_and_privacy(connection, generation_hash)
        repository = repository_type(SingleConnectionPool(connection))
        await _exercise_web_read_role_denial(
            connection,
            repository,
            emit,
            cutoff,
        ...
        await _exercise_head_of_line_claim(
            connection,
            repository,
            first_id,
            second_id,
