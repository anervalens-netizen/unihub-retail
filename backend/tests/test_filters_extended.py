...
def test_scoped_clauses_with_filters():
    ...
    assert len(result) == 5
    assert "s.firma = $2" in result
    assert "s.regional = $3" in result
    assert "st.site_code = $4" in result
    assert "st.agent = $5" in result
...
def test_where_clauses_no_filters():
    ...
    assert "import_month = $1" in clauses[0]
    assert params == ["2026-05"]
...
def test_transaction_filter_parts_all_filters():
    ...
    assert len(params) == 6
    assert any("s.firma" in c for c in clauses)
