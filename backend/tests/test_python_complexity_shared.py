...
def test_total_production_function_count_is_3129(l1):
    """Production tree must contain exactly 3129 measured functions.
    ...
    earnings/attendance export adds 5 functions and the calendar/attendance
    summary adds 2, moving the inventory to 3008. The consolidated exact-main
    V2 helpers add 121 measured functions, moving the inventory to 3129.
    The exact snapshot
    rejects unrelated production-function drift.
    """
    metrics = l1.collect_metrics(PR_B1_WORKTREE)
    assert len(metrics) == 3129, (
        f"production count drift: expected 3129, got {len(metrics)}"
    )
