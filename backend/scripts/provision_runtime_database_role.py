...
async def _validate_login_contract(
    owner: Any,
    *,
    role: str,
    database: str,
    expected_principal: str | None,
    migration_contract: bool,
) -> None:
    ...
    service_role = await owner.fetchrow(
        ...
        "rolbypassrls, rolreplication FROM pg_roles WHERE rolname = $1",
        role,
    )
    ...
    if any(bool(service_role[field]) for field in privileged_fields):
        raise RuntimeError("Service LOGIN must not be privileged")
    if bool(service_role["rolinherit"]) == migration_contract:
        raise RuntimeError(
            "Service LOGIN inheritance flag does not match its authority contract"
        )
...
async def _grant_authorities(
    ...
    *,
    ...
    options = (
        "INHERIT FALSE, SET FALSE" if migration_contract else "INHERIT TRUE, SET FALSE"
    )
...
def _expected_memberships(
    ...
    if migration_contract:
        expected[SCHEMA_OWNER_ROLE] = (False, True)
    return expected


async def _verify_membership_contract(
    owner: Any,
    *,
    role: str,
...
async def _configure_runtime_role(
    owner: Any,
    *,
    role: str,
    quoted_role: str,
    ...
    await _validate_login_contract(
        owner,
        role=role,
        database=database,
    ...
    await _grant_authorities(
        owner,
        quoted_role=quoted_role,
        authority_roles=authority_roles,
        migration_contract=migration_contract,
    )
    await _verify_membership_contract(
        owner,
        role=role,
        expected=_expected_memberships(authority_roles, migration_contract),
    )
...
async def provision(
    ...
    *,
    ...
    try:
        checks = await _runtime_checks(runtime, authority_roles, migration_contract)
    finally:
        await runtime.close()
