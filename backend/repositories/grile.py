...
_CURRENT_STATUS_COLUMNS = """
    ...
    error_message, raw_summary, content_sha256, checked_at
"""
...
class GrileRepository:
    ...
    async def get_latest_data_month(self) -> str | None:
        """Ultima luna operationala cu vanzari importate.

        Targetele lunii urmatoare pot fi publicate in avans, dar nu trebuie sa
        mute Grile pe luna noua inaintea primului import de vanzari. Targetele
        sunt doar fallback pentru o instalare fara niciun snapshot de vanzari.
        """
        async with self.pool.acquire() as conn:
            return await conn.fetchval(
                """
                SELECT COALESCE(
                    (SELECT MAX(import_month) FROM reporting_item_month),
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                ...
                run_month = await conn.fetchval(
                    "SELECT run_month FROM grile_runs WHERE id = $1",
                    run_id,
                )
                if not isinstance(run_month, str):
                    raise RuntimeError("Grile run disappeared before status persistence")
                await _upsert_current_status(
                    conn,
                    run_month=run_month,
                    row=row,
                    source="full",
                    source_run_id=run_id,
                    checked_by_sub=checked_by_sub,
                )
    ...
    async def get_current_status(self, month: str, site_code: str) -> asyncpg.Record | None:
        async with self.pool.acquire() as conn:
            return await conn.fetchrow(
                f"""
                SELECT {_CURRENT_STATUS_COLUMNS}
                FROM grile_store_current_status
...
async def _upsert_current_status(
