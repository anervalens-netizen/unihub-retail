#!/usr/bin/env python3
"""Reject repository-managed current release identity.

The authoritative release identity is generated and signed by CI as
RELEASE_MANIFEST.json and promoted by deploy evidence. Tracked repository
metadata must not recreate a competing current-release authority.
"""
...
def _scan_json_tree(
    ...
    *,
    ...
    release_keys = sorted(
        {_normalized(key) for key in value if _is_release_identity_key(key)}
    )
    if release_keys:
        errors.append(
    ...
    if current:
        support_keys = sorted(
            {_normalized(key) for key in value if _is_support_identity_key(key)}
        )
        if support_keys:
            errors.append(
...
def _current_release_path(raw_path: str) -> bool:
    ...
    for part in Path(raw_path).parts:
        ...
        if (
            normalized.startswith(("current", "latest"))
            and normalized.endswith(("release", "releases"))
        ):
            return True
        if (
            normalized.startswith(("release", "releases"))
            and normalized.endswith(("current", "latest"))
        ):
            return True
    return False
...
def release_authority_errors(root: Path = ROOT) -> list[str]:
    return repository_metadata_errors(root) + canonical_authority_errors(root)
