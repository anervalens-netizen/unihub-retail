...
export function useDashboardHistorySelection({
  ...
  months,
  initialSection,
  preferredHistoryMonth,
}: UseDashboardHistorySelectionArgs) {
  const defaultHistoryMonth = preferredHistoryMonth && months.includes(preferredHistoryMonth)
    ? preferredHistoryMonth
    : dashboardPresenters.getDefaultHistoryMonth(currentMonth, months);
  ...
  const handleToggleHistoryMonth = useCallback((month: string) => {
    ...
    if (!isSelected && draftSelectedHistoryMonths.length >= MAX_DASHBOARD_BATCH_MONTHS) return;
  ...
  return {
    activeSection, setActiveSection, historyMonth, selectedHistoryMonths,
    historySelectionLabel, historySelectionSlug, draftSelectedHistoryMonths,
    draftHistorySelectionLabel, historyMonthDropdownOpen, historyMonthDropdownRef,
    handleToggleHistoryMonth, handleApplyHistoryMonths, handleApplyHistoryPreset,
