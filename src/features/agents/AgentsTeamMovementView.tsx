    ...
    <div className="rounded-2xl border border-slate-200 bg-white/95 p-3 shadow-xl backdrop-blur-md dark:border-slate-700 dark:bg-slate-900/95">
      ...
      {point?.is_baseline && (
        <p className="mb-2 max-w-56 text-[11px] font-medium leading-snug text-slate-500">
          Luna de start pentru tracking pe agent. Nu este tratata ca angajare
          masiva.
        </p>
      )}
        ...
        {payload.map((entry, index) => (
          <div key={index} className="flex items-center gap-3">
            ...
            <span className="text-sm font-bold text-slate-900 dark:text-white">
              {entry.dataKey === "churned_negative"
...
export function AgentsTeamMovementView({
  currentMonth,
  filterLabel,
  loadingOverview,
  overview,
    ...
    <>
      <div ref={teamSectionRef} className="glass scroll-mt-20 rounded-3xl p-4">
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
          ...
          <MetricCard
            ...
            label="Noi"
            value={overview?.new_count ?? "-"}
            className="bg-emerald-50/50 text-emerald-600 dark:bg-emerald-900/10 dark:text-emerald-400"
          />
          <MetricCard
            icon={<UserCheck size={16} className="text-amber-500" />}
            label="Reactivati"
            value={overview?.reactivated_count ?? "-"}
            className="bg-amber-50/50 text-amber-600 dark:bg-amber-900/10 dark:text-amber-400"
          />
          <MetricCard
            icon={<UserMinus size={16} className="text-rose-500" />}
            label="Iesiti luna"
            value={overview?.left_this_month_count ?? "-"}
            hint="fara vanzari fata de luna trecuta"
            className="bg-rose-50/50 text-rose-600 dark:bg-rose-900/10 dark:text-rose-400"
          />
          <MetricCard
            icon={<TrendingUp size={16} className="text-indigo-500" />}
            label="Retentie"
            value={
              overview?.retention_rate != null
                ? `${overview.retention_rate}%`
                : "-"
            }
            className="bg-indigo-50/50 text-indigo-600 dark:bg-indigo-900/10 dark:text-indigo-400"
          />
        </div>
      </div>
      ...
      <div className="glass rounded-3xl p-4">
        <div className="mb-3">
          ...
          <p className="mt-1 text-xs leading-relaxed text-slate-500">
            Indicatori de stabilitate si trend
          </p>
        </div>
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          <MetricCard
            ...
            label="Total Unici"
            value={overview?.total_unique_agents ?? "-"}
            hint="in sistem"
            className="bg-slate-50/80 dark:bg-slate-800/40"
          />
          <MetricCard
            ...
            label="Vechime Medie"
            value={
              overview?.avg_seniority_months != null
                ? `${overview.avg_seniority_months} luni`
                : "-"
            }
            className="bg-blue-50/50 text-blue-600 dark:bg-blue-900/10 dark:text-blue-400"
          />
          <MetricCard
            icon={<Activity size={16} className="text-purple-500" />}
            label="Stabilitate"
            value={
              overview?.stability_rate != null
                ? `${overview.stability_rate}%`
                : "-"
            }
            hint="> 6 luni vechime"
            className="bg-purple-50/50 text-purple-600 dark:bg-purple-900/10 dark:text-purple-400"
          />
          <MetricCard
            icon={<UserMinus size={16} className="text-rose-500" />}
            label="Iesiti istoric"
            value={overview?.churned_total_count ?? "-"}
            hint="absenti ≥ 2 luni"
            className="bg-rose-50/50 text-rose-600 dark:bg-rose-900/10 dark:text-rose-400"
          />
        </div>
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
        <div className="glass rounded-3xl p-4">
          <div className="mb-3">
            ...
            <p className="mt-1 text-xs leading-relaxed text-slate-500">
              Iesiri de personal pentru snapshot {currentMonth}
            </p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <ChurnCard
              label="Churn luna"
              value={
                churnAnalysis.currentChurnRate != null
                  ? `${churnAnalysis.currentChurnRate.toLocaleString("ro-RO", { maximumFractionDigits: 1 })}%`
                  : "-"
              }
              hint={`${churnAnalysis.currentExited} iesiti`}
              className="bg-rose-50/60 text-rose-600 dark:bg-rose-900/10 dark:text-rose-400"
            />
            <ChurnCard
              label="Net luna"
              value={`${churnAnalysis.currentNetGrowth > 0 ? "+" : ""}${churnAnalysis.currentNetGrowth}`}
              hint="activi vs luna trecuta"
              className={`bg-indigo-50/60 dark:bg-indigo-900/10 ${churnAnalysis.currentNetGrowth < 0 ? "text-rose-600 dark:text-rose-400" : "text-emerald-600 dark:text-emerald-400"}`}
            />
            <ChurnCard
              label="Churn mediu 3 luni"
              value={
                churnAnalysis.avgChurnRate != null
                  ? `${churnAnalysis.avgChurnRate.toLocaleString("ro-RO", { maximumFractionDigits: 1 })}%`
                  : "-"
              }
              className="bg-slate-50/80 dark:bg-slate-800/40"
            />
            <ChurnCard
              label="Iesiri in trend"
              value={churnAnalysis.totalExited}
              hint="din 2025-02 incoace"
              className="bg-slate-50/80 dark:bg-slate-800/40"
            />
          </div>
        </div>
        <div className="glass rounded-3xl p-4">
          <div className="mb-3">
            ...
            <p className="mt-1 text-xs leading-relaxed text-slate-500">
              Magazine cu cele mai multe intrari si iesiri de agenti
            </p>
          </div>
          <div className="space-y-2">
            ...
            {topFluxStores.map((item) => (
              <div
                key={item.site_code}
                className="flex items-center justify-between gap-3 rounded-2xl bg-slate-50 px-3 py-2 dark:bg-slate-800/40"
              >
      ...
      <div className="glass rounded-3xl p-4">
        <div className="mb-4">
          ...
          <p className="mt-1 text-[11px] text-slate-500">
            Intrari, iesiri si efectiv activ. 01.2025 este baseline de tracking.
          </p>
        </div>
