...
type RollingData = Awaited<ReturnType<typeof getAiForecastRolling12>>;

function RollingSummary({ data, metric }: { data: RollingData; metric: AiForecastMetric }) {
  const { summary } = data;
  const modelRun = data.runs[0];
  return (
    <section className="glass rounded-3xl p-4">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0"><div className="flex items-center gap-2"><Bot size={17} className="text-indigo-500" /><h3 className="text-sm font-bold">AI Forecast 12 luni — {summary.start_month} / {summary.end_month}</h3></div><p className="mt-1 text-[11px] leading-relaxed text-slate-500 dark:text-slate-400">Prognoza lunara pentru urmatoarele 12 luni, salvata offline pe magazine active si agregata pe structura curenta.</p></div>
        <div className="grid w-full grid-cols-2 gap-2 rounded-2xl bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-500 dark:bg-slate-800 dark:text-slate-300 sm:w-auto sm:grid-cols-3">
          <div><span className="block text-slate-400">Model</span>{modelRun?.model_mode ?? 'xreg + timesfm'}</div><div><span className="block text-slate-400">Sursă</span>{summary.source_month}</div><div><span className="block text-slate-400">Generat</span>{formatGeneratedAt(modelRun?.generated_at)}</div>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-2.5 lg:grid-cols-6">
        ...
        <Metric label="Delta" value={formatSignedAmount(summary.delta_sales, metric)} className={`p-2.5 ${deltaTone(summary.delta_sales)}`} />
        <Metric label="Delta %" value={formatPercent(summary.delta_pct)} className={`p-2.5 ${deltaTone(summary.delta_sales)}`} />
        <Metric label="Magazine" value={formatInt(summary.store_count)} className="p-2.5" /><Metric label="Luni" value={formatInt(summary.month_count)} className="p-2.5" />
      </div>
      {summary.actual_sales === null && <p className="mt-3 text-[11px] text-slate-500 dark:text-slate-400">Nu exista inca realizat pentru lunile viitoare; delta va deveni relevanta dupa importurile lunare.</p>}
    </section>
  );
}

function RollingStores({ data, filteredStores, metric, search, onSearch }: { data: RollingData; filteredStores: RollingData['stores']; metric: AiForecastMetric; search: string; onSearch: (value: string) => void }) {
  const amountFormat = metric === 'units' ? 'integer' as const : 'currency' as const;
  return (
    <section className="glass rounded-3xl p-4">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        <div><div className="flex items-center gap-2"><Building2 size={16} className="text-indigo-500" /><h3 className="text-sm font-bold">Magazine</h3></div><p className="text-[11px] text-slate-500">{filteredStores.length} din {data.stores.length} magazine in forecast.</p></div>
        <div className="flex flex-wrap items-center gap-2">
          <label className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs dark:border-slate-700 dark:bg-slate-900"><Search size={14} className="text-slate-400" /><input value={search} onChange={(event) => onSearch(event.target.value)} placeholder="Cauta magazin" className="w-40 bg-transparent outline-none" /></label>
          <ExportTableButton filename={`ai_forecast_rolling_12_${metric}_magazine`} sheetName="AI Forecast 12 luni" rows={filteredStores} columns={[
            { header: 'Firma', value: (row) => row.firma }, { header: 'Magazin', value: (row) => row.locatie }, { header: 'ASM', value: (row) => row.asm },
            { header: 'Forecast', value: (row) => row.forecast_sales, format: amountFormat }, { header: 'Realizat', value: (row) => row.actual_sales, format: amountFormat },
            { header: 'Delta', value: (row) => row.delta_sales, format: amountFormat }, { header: 'Delta %', value: (row) => row.delta_pct, format: 'percentPoints' },
          ]} />
        </div>
      </div>
...
export function RollingForecastView({ currentMonth, filters, metric }: ForecastViewProps) {
  ...
  return (
    <div className="space-y-3">
      <RollingSummary data={data} metric={metric} />
      ...
      <RollingStores data={data} filteredStores={filteredStores} metric={metric} search={storeSearch} onSearch={setStoreSearch} />
    </div>
  );
}
