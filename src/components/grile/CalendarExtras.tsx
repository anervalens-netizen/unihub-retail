...
import { agentLabel, conflictingCalendarDay, dayChanges, monthDays } from './calendarModel';
...
export function leaveChanges(data: CalendarData, site: string, code: string, from: string, until: string) {
  ...
  if (!data.roster.some(r => r.agent_code === code && r.active && days.every(date => homeOn(r, date) === site))) throw new Error('Alege un agent de bază activ.');
...
export function CalendarExtras({ data, store, stores, writable }: { data: CalendarData; store: CalendarStore; stores: CalendarStore[]; writable: boolean }) {
  ...
  return <div className="space-y-3">
    <section className="overflow-hidden rounded-2xl border border-amber-200 dark:border-amber-800"><h4 className="bg-amber-600 px-3 py-2 font-bold text-white">Concedii · agenții magazinului</h4><div className="p-3">
    ...
    <section className="overflow-hidden rounded-2xl border border-violet-200 dark:border-violet-800"><h4 className="bg-violet-600 px-3 py-2 font-bold text-white">Suplimentari în această locație</h4><div className="p-3">
...
export function SupplementEditor({ data, stores, fixedStore, fixedAgent }: { data: CalendarData; stores: CalendarStore[]; fixedStore?: CalendarStore; fixedAgent?: string }) {
  ...
  const region = fixedStore?.regional || selectedAgent?.regional || stores.find(s => s.site_code === (selectedAgent ? homeOn(selectedAgent, date || rosterDate(data.month)) : undefined))?.regional;
  const candidates = data.roster.filter(r => r.active && (!fixedStore || r.regional === region || stores.some(s => s.site_code === homeOn(r, date || rosterDate(data.month)) && s.regional === region)));
