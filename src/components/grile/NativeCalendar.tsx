...
function Roster({ month, store, data, writable, onChanged }: { month: string; store: CalendarStore; data: CalendarData; writable: boolean; onChanged: () => void }) {
  ...
  const confirm = useMutation({ mutationFn: () => confirmCalendarAgent(month, code, { home_site_code: store.site_code, active, expected_revision: revision }), onSuccess: async () => { onChanged(); await cache.invalidateQueries({ queryKey: ['native-calendar', month] }); setSelection({ code: '', revision: 0, active: true }); } });
