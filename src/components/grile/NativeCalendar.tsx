...
function Roster({ month, store, data, writable }: { month: string; store: CalendarStore; data: CalendarData; writable: boolean }) {
  ...
  const confirm = useMutation({ mutationFn: () => confirmCalendarAgent(month, code, { home_site_code: store.site_code, active: true, expected_revision: 0 }), onSuccess: async () => { await cache.invalidateQueries({ queryKey: ['native-calendar', month] }); setCode(''); } });
