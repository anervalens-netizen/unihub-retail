...
const data: CalendarData = { month: '2026-09', roster: [], days: [], attendance: [], attendance_days: [], closures: [], attendance_by_store: {}, store_hours: [], projection_revision: 'r1' };
...
it('deactivates an existing TL using the selected revision', async () => {
  mount({ ...data, roster: [{ month: '2026-09', agent_code: 'LEADER', display_name: null, transfers: [], identity_status: 'unavailable', home_site_code: 'TL', regional: 'R1', active: true, revision: 3 }] });
...
it('retains a retired region while deactivating its existing leader', async () => {
  mount({ ...data, roster: [{ month: '2026-09', agent_code: 'LEADER', display_name: null, transfers: [], identity_status: 'unavailable', home_site_code: 'TL', regional: 'RETIRED', active: true, revision: 3 }] });
  await userEvent.selectOptions(screen.getByLabelText('Team Leader confirmat'), 'LEADER');
