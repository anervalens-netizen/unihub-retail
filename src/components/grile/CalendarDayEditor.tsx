...
export function CalendarDayEditor({ data, store, stores, date, busy, writable, onSave }: {
  ...
  const blocked = (store.cleanupOnly && (status !== 'cancelled' || !assignedHere(code))) || !code || busy || !writable || Boolean(elsewhere) || (away && (status === 'leave' || status === 'off')) || (status === 'cancelled' && !existing);
  return <form className="space-y-3 rounded-xl bg-slate-50 p-4 dark:bg-slate-800" onSubmit={e => { e.preventDefault(); if (!blocked) onSave(dayChanges(snapshot, { work_date: date, agent_code: code, site_code: store.site_code, status, supplemental: status === 'work' && supplemental })); }}>
    <h4 className="font-semibold">Program pentru {date}</h4>
    <label className="native-label">Agent<select aria-label="Agent pentru zi" className="native-field" value={code} onChange={e => { setCode(e.target.value); setSupplemental(snapshot.days.find(d => d.agent_code === e.target.value && d.work_date === date && d.site_code === store.site_code)?.supplemental ?? false); }}><option value="">Alege agentul</option>{roster.map(r => <option key={r.agent_code} value={r.agent_code}>{agentLabel(r)}{r.home_site_code === store.site_code ? '' : ` · bază ${r.home_site_code}`}</option>)}</select></label>
    ...
    {status === 'work' && <label className="block"><input type="checkbox" checked={supplemental} onChange={e => setSupplemental(e.target.checked)} /> Zi suplimentară plătită</label>}
