...
export function CalendarDayEditor({ data, store, stores, date, busy, writable, onSave, onClose, onCloseDay, anchor }: {
  ...
  const blocked = busy || !writable || Boolean(elsewhere) || (store.virtualBase && !code) || (store.cleanupOnly && !assignedHere(code));
  const form = <form className="space-y-3 rounded-xl bg-slate-50 p-4 dark:bg-slate-800" onSubmit={e => { e.preventDefault(); if (blocked) return; if (closed) { onCloseDay?.({ work_date: date, site_code: store.site_code, closed: true, expected_revision: closure?.revision ?? 0 }, occupant); return; } onSave(dayChanges(snapshot, { work_date: date, agent_code: code, site_code: store.site_code, status: 'work', supplemental: away })); }}>
    ...
    {occupant && <p>Înregistrare actuală: {agentLabel(snapshot.roster.find(r => r.agent_code === occupant.agent_code) ?? occupant)} · lucrează</p>}
