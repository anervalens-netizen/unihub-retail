...
export function CalendarDayEditor({ data, store, stores, date, busy, writable, onSave }: {
  ...
  return <form className="space-y-3 rounded-xl bg-slate-50 p-4 dark:bg-slate-800" onSubmit={e => { e.preventDefault(); if (!blocked) onSave(dayChanges(snapshot, { work_date: date, agent_code: code, site_code: store.site_code, status, supplemental: status === 'work' && (away || supplemental) })); }}>
    ...
    <label className="block">Agent<select aria-label="Agent pentru zi" className="ml-2 rounded border p-2 dark:bg-slate-900" value={code} onChange={e => { setCode(e.target.value); setSupplemental(snapshot.days.find(d => d.agent_code === e.target.value && d.work_date === date && d.site_code === store.site_code)?.supplemental ?? false); }}><option value="">Alege agentul</option>{roster.map(r => <option key={r.agent_code} value={r.agent_code}>{r.agent_code}{r.home_site_code === store.site_code ? '' : ` · suplimentar din ${r.home_site_code}`}</option>)}</select></label>
    {existing && <p>Înregistrare actuală: {dayLabels[existing.status]} · {existing.site_code}</p>}
    <label className="block">Tip zi<select aria-label="Tip zi" className="ml-2 rounded border p-2 dark:bg-slate-900" value={status} onChange={e => setStatus(e.target.value as typeof status)}>{Object.entries(dayLabels).filter(([value]) => !store.cleanupOnly || value === 'cancelled').map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
