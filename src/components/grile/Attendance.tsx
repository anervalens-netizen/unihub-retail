...
export function Attendance({ data, store }: { data: CalendarData; store: CalendarStore }) {
  ...
  const codes = [...new Set([...data.roster.filter(r => r.active && r.home_site_code === store.site_code).map(r => r.agent_code), ...days.map(d => d.agent_code)])].sort();
  const { dates } = monthDays(data.month);
  return <div><p className="mb-3 text-sm">Pontaj provizoriu · ore la locația efectivă · CO = concediu. Numele provin din identități salariale confirmate; codul agentului rămâne vizibil.</p>
    <div className="overflow-x-auto"><table className="w-full border-collapse text-center text-sm"><thead><tr><th className="min-w-44 border p-2">Nume / Cod agent</th>{dates.map(date => <th key={date} className="border p-2">{Number(date.slice(-2))}</th>)}<th className="border p-2">Total ore</th></tr></thead><tbody>{codes.map(code => <Fragment key={code}>{['Ore', 'Interval', 'Pauză'].map((label, index) => <tr key={label}><th className="whitespace-nowrap border p-2 text-left">{index === 0 ? agentLabel(data.roster.find(r => r.agent_code === code) ?? { agent_code: code }) : label}</th>{dates.map(date => {
