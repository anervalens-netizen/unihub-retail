...
function SalaryDrawerHeader({ fullName, history, onClose }: Pick<Props, 'fullName' | 'onClose'> & { history: SalaryAgentHistory | null }) {
  ...
  return (
    <div className="flex items-start justify-between border-b border-slate-200 px-6 py-5 dark:border-slate-700">
      ...
      <button onClick={onClose} className="rounded-lg p-1.5 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600 dark:hover:bg-slate-800" aria-label="Închide istoricul salarial"><X size={20} /></button>
    </div>
  );
}
  ...
  return (
    ...
    >
      <div className="animate-slide-in-right flex h-full w-full max-w-md flex-col bg-white/95 dark:bg-slate-900/95 shadow-2xl">
        <SalaryDrawerHeader fullName={fullName} history={history} onClose={onClose} />
        <div className="flex-1 overflow-y-auto px-6 py-5">
          {loading && <div className="flex items-center justify-center py-16"><RefreshCw size={24} className="animate-spin text-indigo-500" /></div>}
          {!loading && history && <SalaryHistoryContent history={history} />}
          {!loading && !history && <div className="flex flex-col items-center justify-center py-16 gap-3"><p className="text-slate-500">Nu s-au putut încărca datele</p><button onClick={() => fetchSalaryAgentHistory(personId).then(setHistory).catch(() => setHistory(null))} className="rounded-lg bg-indigo-500 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-600">Retry</button></div>}
        </div>
      </div>
