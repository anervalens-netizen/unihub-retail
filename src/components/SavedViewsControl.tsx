...
function useSavedViewsModel(currentState: RetailContextUrlState | null, visible: boolean, navigate?: (url: string) => void) {
  ...
  return { apply, deletingId, error, load, loading, name, ready, remove, save, saving, setName, views };
}
...
function SavedViewsPanel({ model, mode }: { model: SavedViewsModel; mode: 'desktop' | 'mobile' }) {
  return <div className={mode === 'desktop' ? 'w-80 space-y-3' : 'space-y-3'}>
    <form className="flex gap-2" onSubmit={(event) => { event.preventDefault(); void model.save(); }}>
