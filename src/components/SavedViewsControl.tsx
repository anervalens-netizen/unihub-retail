...
import {
  createSavedView, deleteSavedView, listSavedViews, toRetailContextUrlState, type SavedViewItem,
} from '../api/savedViews';
...
function useSavedViewsModel(currentState: RetailContextUrlState | null, visible: boolean, navigate?: (url: string) => void) {
  ...
  const load = useCallback(async () => {
    ...
    try { setViews(await listSavedViews()); setReady(true); }
    catch (caught) { setError(getApiErrorMessage(caught, 'Vederile salvate nu au putut fi încărcate.')); }
  ...
  const save = async () => {
    ...
    try {
      ...
      setName('');
    } catch (caught) { setError(getApiErrorMessage(caught, 'Vederea nu a putut fi salvată.')); }
  ...
  const remove = async (view: SavedViewItem) => {
    ...
    try { await deleteSavedView(view.id); setViews((previous) => previous.filter((item) => item.id !== view.id)); }
    catch (caught) { setError(getApiErrorMessage(caught, 'Vederea nu a putut fi ștearsă.')); }
...
export function SavedViewsControl({ currentState, mode, navigate }: SavedViewsControlProps) {
  ...
  const model = useSavedViewsModel(currentState, mode === 'mobile' || open, navigate);
